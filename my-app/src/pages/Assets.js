...
function Assets() {
  ...
  const handleDelete = (assetToDelete) => {
    axios
      .delete(`http://localhost:3000/Asset/${assetToDelete.AssetID}`)
      .then(() =>{
        setAssets(assets.filter((asset) => asset.AssetID !== assetToDelete.AssetID))
...
<div className="flex flex-col md:flex-row items-center justify-between space-y-3 md:space-y-0 md:space-x-4 p-4">
    ...
    <thead className="text-xs text-gray-700 uppercase bg-gray-50 ">
      <tr>
        ...
        <th scope="col" className="px-4 py-3">ID</th> {/*هذا حق الاي دي للجهاز*/}
      ...
      {assets.length > 0 ? (
        assets.map((asset, i) => (
          <tr className="border-b" key={i}>
            ...
            <td className="px-4 py-3">{asset.AssetID}</td> {/*هذا حق الاي دي للجهاز*/}
